...
//För raspberry-version
  top5Path = 'http://217.210.174.92/getTop5';
...
/*
  top5Path = 'http://localhost:3001/getTop5';
...
*/
  ...
  getTop5() {
    return this.http.get(this.top5Path).pipe(map(res => {
      return res;
    }));
  }
  getKarallen() {
  ...
  deleteFood(data){
    ...
    // this.http.post('http://localhost:3001/delete',
     {id: data._id}).subscribe((res) => {
          console.log(res);
        },
          err => console.log(err)
        );
